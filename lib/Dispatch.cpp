...
static const int MAX_WAIT_TIME = 1 * 60 * 1000;
static const int MAX_IDLE_TIMER = 5 * 1000;
static const int MIN_IDLE_TIMER = 1 * 1000;
...
void Dispatcher::wake(uint tasks, bool master) {
	...
	static uint wake_max = Processor::count() + 1;

	while (tasks && rlist) {
	    if (waking >= wake_max - (master ? 0 : 1))
		break;
	    waking++;
	    lock.unlock();
	    if (!threads || lifo.set()) {
...
void DispatchObj::erase(void) {
    if (flags & (DSP_Active | DSP_ReadyAll | DSP_SelectAll))
	cancel();
    if (flags & DSP_Active)
	flags |= DSP_Freed;
    else
