...
#if !defined(_WIN32) && !defined(DSP_POLL)
...
Dispatcher::Dispatcher(const Config &config): cfg(config),
    ...
    rsock(SOCK_STREAM), wsock(SOCK_STREAM),
#else
    evtfd(-1),
#ifdef DSP_EPOLL
    wfd(-1),
...
bool Dispatcher::exec() {
	...
	if (UNLIKELY((group = obj->group) != NULL)) {
	    if (group->active) {
...
#if !defined(THREAD_PAUSE)
    static constexpr uint SPIN_COUNT = 0;
#elif defined(DSP_KQUEUE)
    static constexpr uint SPIN_COUNT = 32;
#else
...
int Dispatcher::onStart() {
...
#elif defined(DSP_EPOLL)
    if ((wfd = eventfd(0, EFD_CLOEXEC | EFD_NONBLOCK | EFD_SEMAPHORE)) == -1) {
	close(evtfd);
	return -1;
    }
	...
	for (u = 0; u < oeset.size(); u++) {
	    ...
	    olock.lock();
	    flags = ds->flags;
	    if (flags & (DSP_SelectRead | DSP_SelectWrite)) {
		bool r = flags & DSP_SelectRead;
		bool w = flags & DSP_SelectWrite;
...
void Dispatcher::handleEvents(const void *evts, uint nevts) {
    ...
    uint rcnt = 0, rsz;
    bool rset = false;
    uint_fast32_t s = scanning.load(memory_order_relaxed);
	...
	if (diff > 2)
	    diff = 2;
#endif
	// pre-release threads enough for 1/3 of events
	if (scanning.compare_exchange_weak(s, s + (uint_fast32_t)diff,
	    memory_order_acquire, memory_order_relaxed)) {
	    diff = lifo.set(diff);
	    if (diff)
		scanning.fetch_sub(diff, memory_order_relaxed);
	}
    }
    // phase 1: identify sockets from events
	...
	for (;;) {
	    ...
	    uint wake = (rsz > s) ? min(rcnt, (uint)(rsz - s)) : 0;
		...
		if (unwoken) {
		    if (UNLIKELY(workers < maxthreads && !shutdown)) {
			uint spawn = min(unwoken, maxthreads - (uint)workers);
			Thread *t;

			scanning.fetch_sub(unwoken - spawn,
			...
			scanning.fetch_sub(unwoken, memory_order_relaxed);
		    }
		}
		break;
	    }
...
DispatchTimer *Dispatcher::handleTimers(msec_t now) {
	...
	while (cnt < BATCHSZ && (dts[cnt] = timers.get(now)) != NULL)
	    cnt++;
	if (!cnt)
	    break;
	tlock.unlock();
...
void Dispatcher::wakeup(ulong msec) {
	...
	EV_SET(&evt, 1, EVFILT_USER, EV_ENABLE | EV_ONESHOT, NOTE_TRIGGER, 0,
	    NULL);
...
void Dispatcher::pollSocket(DispatchSocket &ds, ulong timeout, DispatchMsg m) {
...
#ifdef DSP_POLL
    slock.lock();
    if (m == DispatchRead || m == DispatchReadWrite || m == DispatchAccept ||
	m == DispatchClose)
	rset.set(ds.fd());
    if (m == DispatchWrite || m == DispatchReadWrite || m == DispatchConnect)
	wset.set(ds.fd());
    slock.unlock();
#elif defined(DSP_DEVPOLL)
	...
	if ((flags & DSP_SelectRead) && m != DispatchReadWrite) {
	    EV_SET(&chgs[nevts++], ds.fd(), EVFILT_READ, EV_DISABLE, 0, 0, &ds);
	}
    }
    if (tmt) {
...
void Dispatcher::ready(DispatchObj &obj, bool hipri) {
	...
	if (rsz <= s + 2)
	    return;
	if (scanning.compare_exchange_strong(s, s + 1, memory_order_acquire,
	    memory_order_relaxed)) {
	    if (!lifo.set()) {
		return;
